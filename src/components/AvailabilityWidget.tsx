  ...
  return <Card className="w-full">
      ...
      <CardContent className="bg-green-200">
        {loading ? <div className="flex justify-center items-center h-24">
          ...
          </div> : slots.length === 0 ? <p className="text-center text-gray-500 py-4">No time slots available for this date</p> : <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {slots.map((slot, index) => <div key={index} className={`border rounded-md p-2 text-center ${slot.is_available ? 'border-green-300 bg-green-50' : 'border-red-300 bg-red-50'}`}>
                <p className="text-sm font-medium">{formatTime(slot.start_time)} - {formatTime(slot.end_time)}</p>
                <Badge className={`mt-1 ${slot.is_available ? 'bg-green-100 text-green-800 hover:bg-green-100' : 'bg-red-100 text-red-800 hover:bg-red-100'}`}>
