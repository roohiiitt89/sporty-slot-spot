...
export const AdminHelpChatInterface: React.FC<AdminHelpChatInterfaceProps> = ({
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      try {
        ...
        const {
          data,
          error
        } = await supabase.from('messages').select('*').eq('user_id', selectedRequest.user_id).is('venue_id', null).order('created_at', {
          ascending: true
        });
        ...
        const unreadMessages = data?.filter(msg => !msg.is_read && msg.sender_id === msg.user_id);
    ...
    const channel = supabase.channel(`help_chat_${selectedRequestId}`).on('postgres_changes', {
      event: 'INSERT',
      schema: 'public',
      table: 'messages',
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const {
        ...
        sender_id: user.id,
        // This is the admin's ID who is replying
        venue_id: null,
        // Help messages have null venue_id
  ...
  return <>
      ...
      <div className="p-4 border-b bg-indigo/10 flex justify-between items-center">
        ...
        {selectedRequest.status !== 'resolved' && <Button onClick={onMarkResolved} variant="outline" className="text-green-600 border-green-300 hover:bg-green-50 hover:text-green-700" size="sm">
      ...
      <div className="flex-grow overflow-y-auto p-4 space-y-4">
        {loading ? <div className="flex justify-center items-center h-full">
          ...
          </div> : messages.length === 0 ? <div className="text-center py-8 text-gray-500">
            <p>No messages yet in this conversation</p>
          </div> : messages.map(message => <div key={message.id} className="flex flex-col">
              <div className={`p-3 rounded-lg max-w-[85%] ${isAdminMessage(message) ? 'bg-indigo text-white self-end' : 'bg-indigo-light/10 self-start'}`}>
                <p className={isAdminMessage(message) ? 'text-white' : 'text-gray-800'}>
                  {message.content}
                </p>
                <span className={`text-xs mt-1 block ${isAdminMessage(message) ? 'text-white/70' : 'text-gray-500'}`}>
      ...
      <form onSubmit={handleSendMessage} className="p-4 border-t">
        <div className="flex">
          <input type="text" value={newMessage} onChange={e => setNewMessage(e.target.value)} placeholder="Type your reply..." disabled={sending || selectedRequest.status === 'resolved'} className="flex-grow border rounded-l-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-indigo" />
          <Button type="submit" className="bg-indigo hover:bg-indigo-dark rounded-l-none" disabled={sending || !newMessage.trim() || selectedRequest.status === 'resolved'}>
            {sending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
          </Button>
        </div>
        {selectedRequest.status === 'resolved' && <p className="text-sm text-yellow-600 mt-2">
