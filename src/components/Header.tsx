        ...
        <div className="flex justify-between items-center">
          <Link to="/" className="flex items-center">
            <span className={`text-2xl font-bold transition-colors duration-300 ${
              isScrolled ? 'text-indigo' : 'text-white'
            }`}>
              SportySlot
            </span>
          </Link>
            ...
            {user ? (
                ...
                <div className="relative">
                  ...
                  {isProfileOpen && (
                    <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-50">
                      <Link 
                        to="/profile" 
                        className="block px-4 py-2 text-sm text-navy-dark hover:bg-slate-light"
              ...
              <>
                ...
                <Link to="/register" className={`nike-button ${
                  isScrolled ? 'bg-indigo text-white' : 'bg-white text-indigo'
                }`}>
                  Sign Up
                </Link>
              </>
          ...
          <button 
            ...
            className={`md:hidden ${isScrolled ? 'text-navy-dark' : 'text-white'}`}
          >
            {isMobileMenuOpen ? (
              <X className="w-6 h-6" />
            ) : (
          ...
          <div className="container mx-auto px-4 py-4 flex flex-col space-y-4">
            ...
            {user ? (
              <>
                ...
                <button
                  onClick={handleSignOut}
                  className="flex items-center font-medium text-red-600 hover:text-red-800 py-2"
                >
                  <LogOut className="w-4 h-4 mr-1" />
                  Sign Out
                </button>
