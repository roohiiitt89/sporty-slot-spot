...
const Header: React.FC = () => {
  ...
  return <header className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white shadow-md py-2' : 'bg-transparent py-4'}`}>
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center">
          ...
          <>
            ...
            {user ? <>
                {!isAdminUser && <Link to="/bookings" className={`flex items-center font-medium transition-colors duration-300 ${isScrolled ? 'text-navy-dark hover:text-indigo' : 'text-white hover:text-indigo-light'}`}>
                ...
                <div className="relative">
                  ...
                  {isProfileOpen && <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-50">
                      {!isAdminUser && <Link to="/profile" className="block px-4 py-2 text-sm text-navy-dark hover:bg-slate-light" onClick={() => setIsProfileOpen(false)}>
                          My Profile
                        </Link>}
                      {isAdminUser && <Link to="/admin" className="flex items-center px-4 py-2 text-sm text-navy-dark hover:bg-slate-light" onClick={() => setIsProfileOpen(false)}>
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && <div className="md:hidden fixed top-[61px] left-0 right-0 bg-white shadow-lg z-40">
        ...
        <>
            ...
            {user ? <>
                {!isAdminUser && <Link to="/bookings" className="flex items-center font-medium text-navy-dark hover:text-indigo py-2" onClick={toggleMobileMenu}>
                    <CalendarDays className="w-4 h-4 mr-1" />
                    My Bookings
                  </Link>}
                ...
                {!isAdminUser && <Link to="/profile" className="flex items-center font-medium text-navy-dark hover:text-indigo py-2" onClick={toggleMobileMenu}>
                    <User className="w-4 h-4 mr-1" />
                    My Profile
                  </Link>}
