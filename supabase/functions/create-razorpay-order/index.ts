...
export default serve(async (req) => {
  ...
  try {
    ...
    let platformFee = 7.0; // default platform fee percent
    let venueSharePercent = 93; // default venue share percent
    ...
    if (venueId) {
      try {
        ...
        if (!supabaseUrl || !supabaseKey) {
          ...
          const venueResponse = await fetch(`${supabaseUrl}/rest/v1/venues?id=eq.${venueId}&select=razorpay_account_id,name,platform_fee_percent`, {
            headers: {
              "apikey": supabaseKey,
          ...
          if (venueResponse.ok) {
            ...
            if (venueData && venueData.length > 0) {
              ...
              // Fetch platform fee percent, default to 7 if not set
              platformFee = venueData[0].platform_fee_percent ?? 7.0;
              venueSharePercent = 100 - platformFee;
    ...
    if (venueAccountId) {
      ...
      const venueShare = Math.floor(amountInPaise * (venueSharePercent / 100));
      ...
      transfers = [
        {
          ...
          notes: {
            ...
            share_percentage: `${venueSharePercent}%`
          }
        }
      ];
      
      console.log(`Setting up transfer to venue account ${venueAccountId} of amount ${venueShare} (${venueSharePercent}% of total ${amountInPaise})`);
    } else if (venueId) {
